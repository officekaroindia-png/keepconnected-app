...
// IST today — matches the server's localDate() so the timeline opens the right day
const todayIST = () =>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={{ padding: SP.lg, paddingBottom: 40 }}
        ...
        {rows.map((p, i) => {
          ...
          const openTimeline = () => navigation.navigate('MyDayTimeline', {
            date: todayIST(), id: p.id, name: p.name, mobile: p.mobile,
          });
          return (
            <Animated.View key={p.id} entering={FadeInDown.delay(25 * i).springify().damping(18)} style={[styles.card, SHADOW.card]}>
              <Pressable style={styles.cardMain} onPress={openTimeline}>
                  ...
                  <View style={styles.nameRow}>
                    ...
                    {p.offsiteAuthToday && (
                      <View style={styles.offsitePill}>
                        ...
                        <Text style={styles.offsiteTxt}>Offsite</Text>
                      </View>
                    )}
                  </View>
...
const styles = StyleSheet.create({
  ...
  card: { flexDirection: 'row', alignItems: 'center', gap: SP.sm, backgroundColor: COLORS.surface, borderRadius: R.md, borderWidth: 1, borderColor: COLORS.border, marginBottom: SP.sm, overflow: 'hidden' },
  cardMain: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: SP.md, padding: SP.md },
  ...
  offsitePill: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: '#E0F2FE', borderRadius: R.pill, paddingHorizontal: 6, paddingVertical: 2 },
  offsiteTxt: { fontSize: 9, fontWeight: '800', color: '#0891B2' },
  ...
  iconBtn: { width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center', marginRight: SP.sm },
